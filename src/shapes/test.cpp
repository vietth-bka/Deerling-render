// constexpr int NumBins = 16; // Number of bins for the SAH calculation
        ...
        // bestSplitAxis = -1; // Default to no useful split
        // float bestCost = std::numeric_limits<float>::infinity();
        // bestSplitPosition = 0;

        // if (node.aabb.isEmpty() || node.primitiveCount <= 0) {
        //     return; // No valid split is possible
        // }
        ...
        //         continue; // Skip axis with no range
        //     }

        //     float scale = NumBins / range;
        //     Bin bins[NumBins];

        //     // First pass: assign primitives to bins
        //     for (NodeIndex i = 0; i < node.primitiveCount; ++i) {
        //         int primitiveIndex = m_primitiveIndices[node.leftFirst + i];
        //         if (primitiveIndex < 0) { // Add a safeguard for negative indices
        //             throw std::runtime_error("Primitive index is invalid");
        //         }
        //         Point centroid = getCentroid(primitiveIndex);
        //         int binIndex = std::min(std::max(0, static_cast<int>((centroid[axis] - minCoord) * scale)), NumBins - 1);

        //         bins[binIndex].count++;
        //         bins[binIndex].bounds.extend(getBoundingBox(primitiveIndex));
        //     }

        //     // Compute cumulative bounds and primitive counts
        ...
        //     if (totalSurfaceArea <= std::numeric_limits<float>::epsilon()) {
        //         continue; // Skip axis with invalid surface area
        //     }
...
class AccelerationStructure : public Shape {
    ...
    void binning(const Node &node, int &bestSplitAxis,
        ...
        for (int axis = 0; axis < node.aabb.Dimension; ++axis){
			float min_val = node.aabb.min()[axis];
			float max_val = node.aabb.max()[axis];
            float distance = max_val - min_val;

			if (distance <= Epsilon) {continue;}

            float scale = distance / numBin;

			Bin bins[numBin] = {};
			for (int i = 0; i < node.primitiveCount; ++i){
				int primitiveIndex = m_primitiveIndices[node.leftFirst + i];
				Point cent = getCentroid(primitiveIndex);
				int binID = int((cent[axis] - min_val) / scale);
				binID = clamp(binID, 0, numBin-1);
				
				bins[binID].count++;
				bins[binID].bound.extend(getBoundingBox(primitiveIndex));
				}
			
			Bin leftBins[numBin], rightBins[numBin];
			leftBins[0] = bins[0];
			rightBins[numBin-1] = bins[numBin-1];

			for (int i = 1; i < numBin; ++i){
				leftBins[i] = leftBins[i-1];
				leftBins[i].bound.extend(bins[i].bound);
				leftBins[i].count += bins[i].count;				
			}

			for (int i = numBin-2; i >= 0; --i){
				rightBins[i] = rightBins[i + 1];
				rightBins[i].bound.extend(bins[i].bound);
				rightBins[i].count += bins[i].count;
			}

			for (int i = 0; i < numBin-1; i++){
				float S_tot = surfaceArea(node.aabb);
				float S_L = surfaceArea(leftBins[i].bound);
				float S_R = surfaceArea(rightBins[i + 1].bound);
				float cost = 0.125f + S_L/S_tot*leftBins[i].count + S_R/S_tot*rightBins[i+1].count;
				
				if (cost < min_cost){
					min_cost = cost;
					bestSplitAxis = axis;
					bestSplitPosition = (i+1) * scale + min_val;
				}
			}
        }

		if (bestSplitAxis == -1){
			bestSplitPosition = (node.aabb.max()[0] + node.aabb.min()[0])/2.f;
		}
