...
/**
 * @brief Returns the Pdf w.r.t solid angle measure to Pdf w.r.t surface area.
 */
inline float SurfaceAreaPDFToSolidAnglePDF(const float &pdf, const float &distance, const Vector &n, const Vector &wi)
    {
        Vector wi_normalized = wi.normalized();
        if (abs(n.dot(-wi_normalized)) <= Epsilon)
            return Infinity;

        float result = pdf * sqr(distance) / abs(n.dot(-wi_normalized));
